...
export const SportsSection = () => {
  ...
  const onDragEnd = () => {
    ...
    if (x <= -DRAG_BUFFER && imgIndex < sportsNews.length - 1) {
      setImgIndex((pv) => pv + 1);
  ...
  return (
    <section className="my-container pt-8">
      ...
      <div className="relative overflow-hidden py-8">
        ...
        >
          <Images imgIndex={imgIndex} news={sportsNews} />
        </motion.div>
      ...
      <div className="mb-10 grid grid-cols-1 md:grid-cols-3 justify-between items-center space-y-4 md:space-y-6 py-6 px-10 md:px-0 group">
          <StarHeader title="More News" className="col-span-full"/>
            {
              sportsNews.slice(3,6).map((post)=>(
                  <div className={`flex items-center justify-center gap-6 w-fit`} key={nanoid()}>
                    ...
                    <Link to={`/category/sports/${titleToSlug(post.title)}`} className="md:max-w-[220px] newsreader-600 line-clamp-3">{post.title}</Link>
                  </div>
              ))
            }
        </div>
        ...
        return (
          ...
          >
            <div className="ml-10 absolute bottom-0 z-20 backdrop-blur-xl  bg-white/80 shadow-xl p-4 -mt-10">
              <Link to={`/category/sports/${titleToSlug(title)}`}>
