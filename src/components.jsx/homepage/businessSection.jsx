      ...
      <div className="my-container mx-auto px-4 pb-4">
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-full md:col-span-6 lg:col-span-8">
            <img className="w-full aspect-video object-cover" src={newsData.business[0].urlToImage} alt={newsData.business[0].title} />
            ...
            <p className="text-gray-700 newsreader-500">{newsData.business[0].description}</p>
            <div className="text-sm text-gray-500">
                  By <span className="font-semibold">{newsData.business[0].author ? newsData.business[0].author : 'anonymous'}</span>
                </div>
            </div>
          </div>
          ...
          <Link to={`/category/business/${titleToSlug(newsData.business[4].title)}`} className="gap-1 items-center grid grid-cols-5 border-b py-3">
            ...
            <img className="w-full border aspect-video col-span-full md:col-span-1 md:aspect-[9/12] object-cover" src={newsData.business[4].urlToImage} alt={newsData.business[4].title} />
            </Link>
            ...
            <img className="w-full border aspect-video col-span-full md:col-span-1 md:aspect-[9/12] object-cover" src={newsData.business[5].urlToImage} alt={newsData.business[5].title} />
            </Link>
          ...
          <StarHeader title="Trending News" className="col-span-full"/>
            {
              newsData.business.slice(1,4).map((post, index)=>(
                  <div className={`flex items-center justify-center gap-6 w-fit`} key={nanoid()}>
                    <h1 className="~text-6xl/8xl relative font-bold italic before:content-[''] before:translate-x-1/2  before:-z-10 before:absolute before:rotate-12 before:w-8 before:h-full before:bg-primary">{index+1}</h1><Link to={`/category/business/${titleToSlug(post.title)}`} className="md:max-w-[220px] newsreader-600">{post.title}</Link>
                  </div>
              ))
            }
        </div>
